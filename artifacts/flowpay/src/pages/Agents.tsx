...
const X402_STEPS = [
  { req: "GET /api/data", res: "402 Payment Required", note: "Agent requests resource", color: "#f87171" },
  { req: "POST /x402/pay  0.001 USDG", res: "TX: Solana ⚡", note: "FlowPay sends micro-payment", color: ACCENT },
  { req: "GET /api/data  X-Payment-Proof", res: "200 OK  { data }", note: "API returns response", color: "#4ade80" },
];
  ...
  return (
    <AppLayout>
      ...
      <div className="relative overflow-hidden" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div className="relative z-10 px-8 pt-8 pb-4">
            ...
            <div className="flex items-start gap-4 min-w-0">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2.5 mb-1.5">
                  ...
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.10)" }}>x402 Protocol</span>
                </div>
                <p className="text-[13px] sm:text-sm" style={{ color: "rgba(255,255,255,0.56)" }}>
                  Autonomous AI agent wallets on Solana — machine-to-machine payments at &lt;500ms
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 pb-7">
            {[
              { label: "Agents Deployed", value: agents.length.toString(), accent: true },
              { label: "Active Now", value: activeCount.toString(), accent: false },
              { label: "USDG Held", value: `$${totalBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, accent: false },
              { label: "Transactions", value: totalTxs.toString(), accent: false },
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${ACCENT}20` }}>
          <div className="px-5 py-3.5 flex items-center gap-3" style={{ borderBottom: `1px solid ${ACCENT}15`, background: `${ACCENT}06` }}>
            <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: ACCENT, boxShadow: `0 0 6px ${ACCENT}` }} />
            <p className="text-[12px] font-semibold" style={{ color: ACCENT }}>x402 Protocol — How Agents Pay for APIs</p>
          </div>
          <div className="p-5">
            <div className="space-y-2">
              {X402_STEPS.map((step, i) => (
                <div key={i} className="flex items-start gap-4">
                  <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 text-[9px] font-bold mt-0.5" style={{ background: `${step.color}20`, color: step.color, border: `1px solid ${step.color}40` }}>
                    {i + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap">
                      <code className="text-[11px] font-mono px-2 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.8)" }}>
                        {step.req}
                      </code>
                      <span className="text-[10px]" style={{ color: "rgba(255,255,255,0.3)" }}>→</span>
                      <code className="text-[11px] font-mono px-2 py-0.5 rounded" style={{ background: `${step.color}10`, color: step.color, border: `1px solid ${step.color}25` }}>
                        {step.res}
                      </code>
                    </div>
                    <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>{step.note}</p>
                  </div>
                </div>
              ))}
            </div>
            <p className="text-[11px] mt-4 pt-4" style={{ color: "rgba(255,255,255,0.3)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              Agents with x402 enabled autonomously pay micro-fees to access APIs — no human intervention required. Built on Solana for sub-second finality.
            </p>
          </div>
        </div>
          ...
          <div className="rounded-2xl p-6" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.09)" }}>
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-[11px] mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>Creates an on-chain wallet and registers on Dodo</p>
              </div>
            ...
            <form onSubmit={handleCreate} className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.4)" }}>Agent Name</label>
                <input value={form.name} onChange={(e) => set("name", e.target.value)} required placeholder="e.g. InvoiceBot"
              ...
              <div>
                <label className="text-[10px] uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.4)" }}>Owner / Team</label>
                <input value={form.ownerName} onChange={(e) => set("ownerName", e.target.value)} required placeholder="Your name or company"
                  className="w-full mt-1 rounded-xl px-3 py-2.5 text-white text-sm outline-none transition-all"
                  style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)" }}
                  onFocus={(e) => (e.target.style.borderColor = `${ACCENT}50`)}
                  onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,0.09)")} />
              ...
              <div className="col-span-2">
                <label className="text-[10px] uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.4)" }}>What does this agent do?</label>
                <textarea value={form.description} onChange={(e) => set("description", e.target.value)} required rows={2}
                  placeholder="Describe its purpose — e.g. 'Pays API providers for data access autonomously'"
              ...
              <div className="col-span-2 flex items-center gap-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="text-[11px]" style={{ color: "rgba(255,255,255,0.35)" }}>Allow agent to autonomously pay for HTTP 402 API access</div>
                  </div>
                </label>
              </div>
              {formError && (
        ...
        <div className="space-y-4">
          {loading ? [...Array(2)].map((_, i) => (
            <div key={i} className="h-48 rounded-2xl animate-pulse" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }} />
          )) : agents.length === 0 ? (
            <div className="rounded-2xl px-6 py-16 text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: `${ACCENT}10`, border: `1px solid ${ACCENT}20` }}>
                <Bot className="w-5 h-5" style={{ color: `${ACCENT}80` }} />
              </div>
              <p className="text-sm font-medium text-white mb-1">No agents deployed yet</p>
              <p className="text-[13px] max-w-sm mx-auto" style={{ color: "rgba(255,255,255,0.35)" }}>
                Deploy your first AI agent wallet — it gets a Solana address and can autonomously pay for APIs via the x402 protocol.
              </p>
            </div>
          ) : agents.map((agent) => {
              ...
              <div key={agent.id} className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        ...
                        {isActive && (
                          <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-[#070707]" style={{ background: "#4ade80", boxShadow: "0 0 6px #4ade80" }} />
                        )}
                      </div>
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={
                            isActive
                              ? { background: "#4ade8015", color: "#4ade80", border: "1px solid #4ade8025" }
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => { setSelectedAgent(isSelected ? null : agent); if (!txs[agent.id]) loadTxs(agent.id); }}
                        className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg transition-all"
                        style={{ background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.6)", border: "1px solid rgba(255,255,255,0.10)" }}>
                        <Activity className="w-3 h-3" /> {isSelected ? "Hide" : "History"}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {selectedAgent?.id === agent.id && action === "fund" && (
                  <div className="px-5 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}>
                    <p className="text-[10px] uppercase tracking-widest mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>Fund Agent Wallet</p>
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => { setAction(null); setActionError(null); }} className="px-3 py-2 text-sm rounded-xl" style={{ background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.4)" }}>✕</button>
                    </div>
                    {actionError && <p className="mt-2 text-xs rounded-lg px-3 py-2" style={{ background: "rgba(248,113,113,0.10)", border: "1px solid rgba(248,113,113,0.20)", color: "#f87171" }}>{actionError}</p>}
                  </div>
                )}
                ...
                {selectedAgent?.id === agent.id && action === "pay" && (
                  <div className="px-5 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}>
                    <p className="text-[10px] uppercase tracking-widest mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>Execute Agent Payment</p>
                    <div className="grid grid-cols-2 gap-2 mb-2">
                      {[
                        ...
                        <input key={key} type={type ?? "text"} value={(payForm as Record<string, string>)[key]}
                    ...
                    {actionError && <p className="mb-2 text-xs rounded-lg px-3 py-2" style={{ background: "rgba(248,113,113,0.10)", border: "1px solid rgba(248,113,113,0.20)", color: "#f87171" }}>{actionError}</p>}
                    <div className="flex gap-2">
                      <button onClick={() => handlePay(agent.id)} disabled={submitting || !payForm.recipientName || !payForm.amountUsdg || !payForm.purpose}
                        className="flex-1 text-sm font-bold py-2 rounded-xl transition-all disabled:opacity-50"
                        style={{ background: ACCENT, color: "#000" }}>
                ...
                {isSelected && txs[agent.id] && (
                  <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                    <div className="px-5 py-2.5 flex items-center gap-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.01)" }}>
                      <Activity className="w-3 h-3" style={{ color: "rgba(255,255,255,0.3)" }} />
                      <p className="text-[10px] uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.3)" }}>Transaction Log</p>
                    </div>
                    <div className="max-h-56 overflow-y-auto divide-y" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                      {txs[agent.id].length === 0 ? (
                        <p className="px-5 py-6 text-xs text-center" style={{ color: "rgba(255,255,255,0.35)" }}>No transactions yet — fund or pay to start</p>
                      ) : txs[agent.id].map((tx) => (
                        <div key={tx.id} className="flex items-center gap-3 px-5 py-3">
                          <div className="p-1.5 rounded-lg shrink-0" style={{ background: tx.type === "fund" ? "rgba(74,222,128,0.10)" : `${ACCENT}10` }}>
                            {tx.type === "fund"
                          ...
                          <div className="text-right shrink-0">
                            <p className="text-xs font-mono font-semibold" style={{ color: tx.type === "fund" ? "#4ade80" : ACCENT }}>
                              {tx.type === "fund" ? "+" : "-"}${parseFloat(tx.amountUsdg).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </p>
                            {tx.settlementMs && <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.3)" }}>{tx.settlementMs}ms</p>}
                          </div>
                          {tx.solanaSignature && (
                            <button onClick={() => navigator.clipboard.writeText(tx.solanaSignature ?? "")} className="transition-colors hover:text-white/60 shrink-0" style={{ color: "rgba(255,255,255,0.15)" }}>
                              <Copy className="w-3 h-3" />
                            </button>
                          )}
                        </div>
