...
import { Users, Plus, CheckCircle, Copy, X, ExternalLink } from "lucide-react";
        ...
        <div className="relative z-10 flex items-start justify-between px-8 pt-8 pb-7 min-w-0 gap-6">
          <div className="flex items-start gap-4 min-w-0">
            ...
            <div className="min-w-0 max-w-[54rem]">
              ...
              <p className="text-[13px] sm:text-sm leading-relaxed max-w-2xl" style={{ color: "rgba(255,255,255,0.56)" }}>Stablecoin payroll for remote teams — settled on Solana in &lt;3s, processed via Dodo</p>
            </div>
          </div>
          <button
        ...
        {success && (
          <div className="rounded-2xl p-5 mb-6" style={{ background: `${ACCENT}08`, border: `1px solid ${ACCENT}25` }}>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {success.dodoPaymentId && (
                    <span className="text-[11px] font-mono px-2 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.45)", border: "1px solid rgba(255,255,255,0.10)" }}>
                      Dodo: {success.dodoPaymentId.slice(0, 16)}…
                    </span>
                  )}
                  {success.dodoCheckoutUrl && (
                    <a
                      href={success.dodoCheckoutUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-lg transition-all hover:opacity-80"
              ...
              <button onClick={() => setSuccess(null)} className="text-white/20 hover:text-white/50 transition-colors shrink-0"><X className="w-4 h-4" /></button>
            </div>
          </div>
        )}
        ...
        {showForm && (
          <div className="rounded-2xl p-6 mb-6" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.09)" }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setShowForm(false); setError(null); }} className="text-white/25 hover:text-white/60 transition-colors"><X className="w-4 h-4" /></button>
            </div>
            <form onSubmit={handleSend} className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={key}>
                  ...
                  <input type={type ?? "text"} value={(form as Record<string, string>)[key]} onChange={(e) => set(key, e.target.value)} required placeholder={placeholder}
              ...
              {error && (
                <div className="col-span-2 rounded-lg px-3 py-2.5 text-sm" style={{ background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.25)", color: "#f87171" }}>
                  {error}
                </div>
              )}
              ...
              <div className="col-span-2 flex gap-3 pt-1">
                <button type="submit" disabled={submitting}
                  ...
                  style={{ background: ACCENT, color: "#000" }}>
                  {submitting ? "Processing via Dodo…" : "Send Payment"}
                </button>
                ...
                <div key={p.id} className="flex items-center gap-4 px-6 py-4 transition-colors hover:bg-white/[0.02]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      <p className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>{p.recipientEmail} · {new Date(p.createdAt).toLocaleString()}</p>
                      {p.dodoPaymentId && (
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.35)", border: "1px solid rgba(255,255,255,0.08)" }}>
                          {p.dodoPaymentId.startsWith("dodo_") ? "mock" : `Dodo: ${p.dodoPaymentId.slice(0, 12)}…`}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {p.dodoCheckoutUrl && (
                      <a href={p.dodoCheckoutUrl} target="_blank" rel="noopener noreferrer"
                        className="transition-colors hover:text-white/60" style={{ color: ACCENT }}
                        title="Open Dodo Checkout">
