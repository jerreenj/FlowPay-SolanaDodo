...
4. The checkout link (`https://test.checkout.dodopayments.com/session/cks_…`) is clickable directly from the UI
...
List any digital product. Share a link (`/buy/:id`) — no account needed to purchase. Each product gets a real Dodo product ID (`pdt_…`). Each purchase creates a real Dodo checkout session and redirects the buyer to `https://test.checkout.dodopayments.com/session/…`.
